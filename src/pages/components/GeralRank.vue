<template>
	<article class="rank-geral">
		...
		<div style="width: 100%" v-else>
			<h2 class="rank-title">Classificação Geral</h2>
			<ol class="podium-rank">
				<li :id="'li-'+(index+1)" v-for="(driver, index) in drivers" :key="index">
					<a :href="'#li-'+(index+1)">
						<div class="container">
							<div class="image">
							...
							<div class="rank-content">
								<h3>{{driver.position}}º Lugar</h3>
								<div class="rank-content-edit">
									<h4>{{driver.name}} {{driver.lastName}}</h4>
									<h5>{{parseInt(driver.points)}}</h5>
								</div>
							</div>
						</div>
					</a>
...
export default {
	...
	methods: {
		rankGeral() {
			...
			.then(response => {
				let allDrivers = response.data.data
				this.drivers.push(allDrivers[0])
				this.drivers.push(allDrivers[1])
				this.drivers.push(allDrivers[2])
				this.drivers.push(allDrivers[3])
				this.drivers.push(allDrivers[4])
				this.loading = false
			})
			.catch(error => {
				this.loading = false
