		...
		<div style="width: 100%" v-else>
			...
			<ol class="podium-rank">
				<li :id="'li-'+(index+1)" v-for="(driver, index) in f1drivers" :key="'f1'+index">
					<a :href="'#li-'+(index+1)">
						<div class="container">
							<div class="driver-image" :style="'background-color:' + driver.teamColor">
...
export default {
	...
	methods: {
		rankGeral() {
			...
			.then(response => {
				const allF1Drivers = response.data.data.filter(function(value, index, arr){
				return value.category === "F1";
			});
				const allF2Drivers = response.data.data.filter(function(value, index, arr){
				return value.category === "F2";
			});
				for(var i = 0; i < 5; i++) {
					this.f1drivers.push(allF1Drivers[i])
					this.f2drivers.push(allF2Drivers[i])
				}
				this.loading = false
			})
			.catch(error => {
